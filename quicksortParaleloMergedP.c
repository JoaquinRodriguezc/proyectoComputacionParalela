...
int main(int argc, char **argv)
{
    ...
    */
    ...
    int activeProcesses = numeroProcesos;
    while (activeProcesses > 1)
    {
        if (processRank < activeProcesses)
        {
            if (processRank % 2 == 0)
            {

                MPI_Send(&localFragmentSize, 1, MPI_INT, (processRank + 1) % numeroProcesos, 0, MPI_COMM_WORLD);
                MPI_Send(localFragment, localFragmentSize, MPI_INT, (processRank + 1) % numeroProcesos, 0, MPI_COMM_WORLD);
            }
            else
            {

                int foreignFragmentSize;
                MPI_Recv(&foreignFragmentSize, 1, MPI_INT, (processRank - 1) % numeroProcesos, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                int *foreignFragment = (int *)malloc(foreignFragmentSize * sizeof(int));
                MPI_Recv(foreignFragment, foreignFragmentSize, MPI_INT, (processRank - 1) % numeroProcesos, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                int *mergedOrderedArray = (int *)malloc(sizeof(int) * (localFragmentSize + foreignFragmentSize));

                /*
                arrays mergeados pero no ordenados
                */
                memcpy(mergedOrderedArray, localFragment, localFragmentSize * sizeof(int));
                memcpy(mergedOrderedArray + foreignFragmentSize, foreignFragment, foreignFragmentSize * sizeof(int));

                int newFragmentSizes[2] = {foreignFragmentSize, localFragmentSize};
                ...
                */
                ...
                free(mergedOrderedArray);
                free(foreignFragment);
            }
        }
        MPI_Barrier(MPI_COMM_WORLD);
        activeProcesses = activeProcesses / 2;
    }
